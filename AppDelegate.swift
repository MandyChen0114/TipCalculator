...
class AppDelegate: UIResponder, UIApplicationDelegate {
        ...
        if let viewControllers = self.window?.rootViewController?.childViewControllers {
            for viewController in viewControllers {
                if viewController is ViewController {
                    let billField = (viewController as! ViewController).billField.text
                    defaults.set(billField, forKey: "lastBillAmount")
                    
                    let lastQuitTime: Date! = Date()
                    defaults.set(lastQuitTime, forKey: "lastQuitTime")
    ...
    func readBillAmount() {
        if defaults.object(forKey: "lastQuitTime") == nil {
            return
        }
        let lastQuitTime = defaults.object(forKey: "lastQuitTime") as! Date
